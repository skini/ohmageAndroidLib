...
public class FilterControl extends LinearLayout {
    ...
    private final Activity mActivity; // stores a reference to our calling
                                      // activity
    private AlertDialog mItemListDialog; // stores a dialog containing a list of
                                         // items, updated by populate() and
                                         // add()
    ...
    /**
     * Constructs the parts of the control that provide actual functionality.
     * Declarative styling is handled by initStyles().
     * 
     * @param context the context of whoever's creating this control, usually
    ...
    protected void initControl(Context context) {
        ...
        // and init an empty list so we don't crash and burn when we try to read
        // before populating
    ...
    /**
     * Initializes the appearance of the control based on the attributes passed
     * from the xml.
     * 
     * @param attrs the collection of attributes, usually provided in the
     ...
     * 
     * @param data the cursor from which to load the data; will be closed when
     *            the method returns.
     * @param textColumn the column of the cursor to use as the displayed text
     *            on the filter
     * @param valueColumn the column of the cursor to return from the
    ...
    /**
     * Replaces the items in the filter's list with an ArrayList of
     * Pair<String,String>.
     * 
    ...
    /**
     * Sets the currently selected item by its index, which should be between 0
     * and list size - 1 inclusive.
     * 
    ...
    /**
     * Gets the displayed text for the currently selected item (i.e. the first
     * element of the Pair)
     * 
    ...
    /**
     * Gets the defined value for the currently selected item (i.e. the second
     * element of the Pair)
     * 
    ...
    /**
     * Sets the currently selected item by its value, if the value exists in the
     * collection of items
     * 
    ...
    /**
     * Attaches a {@link FilterChangeListener} to the filter which will be
     * called when the user navigates between items or when the list is changed.
     * 
     * @param listener an object implementing {@link FilterChangeListener} which
     *            will be called when the list index is changed.
     */
    ...
    /**
     * Exposes a callback to allow custom processing to occur when the filter is
     * changed (either by navigation or population).
     */
    ...
    private void notifyFilterChanged(boolean selfChange) {
        if (mFilterChangeListener != null)
            mFilterChangeListener
                    .onFilterChanged(selfChange, mItemList.get(mSelectionIndex).second);
    }

    /**
     * Handles the next, previous, and current buttons in the view.
     * 
     * @param v the view which generated the click; this method uses the id of
     *            the view to determine what to do
     */
    private class FilterClickHandler implements OnClickListener {
        ...
        public void onClick(View v) {
            Analytics.widget(v);
            int id = v.getId();
            if (id == R.id.controls_filter_prev) {
                if (mSelectionIndex > 0) {
                    mSelectionIndex -= 1;
                    syncState(false);
                }
            } else if (id == R.id.controls_filter_current) {
                mItemListDialog.show();
            } else if (id == R.id.controls_filter_next) {
    ...
    private void syncState(boolean selfChange) {
        ...
        mNextBtn.setTextColor((mSelectionIndex >= mItemList.size() - 1) ? Color.LTGRAY
                : Color.BLACK);
        ...
        // grab the selection so we can populate the middle button and fire a
        // callback
    ...
    // helper method for constructing a list dialog based on the current item
    // list
