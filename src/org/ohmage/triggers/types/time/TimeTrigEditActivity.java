...
public class TimeTrigEditActivity extends PreferenceActivity implements View.OnClickListener,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            ...
            // if there are any preselected actions specified when the activity
            // is first created
    ...
    private void initializeGUI() {
        ...
        if (mTrigDesc.isRangeEnabled()) {
             ...
             * SharedPreferencesHelper.TRIGGERS_TIMERANGE_ALWAYS_RANDOM) {
             * randPref.setChecked(true); } else {
        ...
        ((Button) findViewById(R.id.trig_edit_done)).setEnabled(mAdminMode
                || TrigUserConfig.editTimeTrigger);
    }
    ...
    private void udateTriggerDesc() {
        ...
        if (rangePref.isChecked()) {
             ...
             * SharedPreferencesHelper.TRIGGERS_TIMERANGE_ALWAYS_RANDOM) {
             * mTrigDesc.setRandomized(true); } else {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.trig_edit_done) {
            if (mExitListener != null) {
                ...
                // since we didn't hit any issues, we must be good; invoke the
                // exit handler which stores the trigger
    ...
    private void updateTriggerTimePrefStatus() {
        ...
        // if triggers_timerange_always_random is set, then make randPref =
        // rangePref
        // randPref.setChecked(SharedPreferencesHelper.TRIGGERS_TIMERANGE_ALWAYS_RANDOM
        // && rangePref.isChecked());
    ...
    private Dialog createRepeatSelDialog() {
        ...
        updateRepeatStatusArray();
        mRepeatDialog = new AlertDialog.Builder(this).setTitle(R.string.trigger_time_select_days)
                .setPositiveButton(R.string.done, this).setNegativeButton(R.string.cancel, this)
    ...
    private Dialog createInvalidTimeAlert() {
        return new AlertDialog.Builder(this).setTitle(R.string.trigger_time_invalid_settings)
                .setNegativeButton(R.string.cancel, null)
                .setMessage(R.string.trigger_time_invalid_text).create();
    }

    private Dialog createNoSurveysSelectedAlert() {
        return new AlertDialog.Builder(this).setTitle(R.string.trigger_time_no_survey_selected)
                .setNegativeButton(R.string.cancel, null)
                .setMessage(R.string.trigger_time_no_survey_selected_text).create();
    }
     ...
     * String prevActDesc = mDb.getActionDescription(trigId); TriggerActionDesc
     * prevDesc = new TriggerActionDesc(); prevDesc.loadString(prevActDesc);
     * mDb.updateActionDescription(trigId, desc.toString()); mCursor.requery();
     * Notifier.refreshNotification(this, mCampaignUrn, true);
     * if(desc.getCount() == 0 && prevDesc.getCount() !=0) {
     * toggleTrigger(trigId, false); } if(desc.getCount() != 0 &&
     * prevDesc.getCount() == 0) { toggleTrigger(trigId, true); } }
     */
}
