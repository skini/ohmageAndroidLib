 ...
 * an existing trigger. 
 */
public class LocTrigEditActivity extends PreferenceActivity implements OnPreferenceClickListener,
        OnPreferenceChangeListener, OnClickListener {

    private static final String TAG = "LocTrigEditActivity";

    public static final String KEY_TRIG_DESC = LocTrigEditActivity.class.getName()
            + ".trigger_descriptor";
    public static final String KEY_TRIG_ID = LocTrigEditActivity.class.getName() + ".trigger_id";
    public static final String KEY_ADMIN_MODE = LocTrigEditActivity.class.getName() + ".admin_mode";
    public static final String KEY_ACT_DESC = LocTrigEditActivity.class.getName() + ".act_desc";
    ...
    /*
     * Interface for the callback which is invoked when the done button is
     * pressed.
     */
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (config != null) {
            ...
            if (mTrigDesc.loadString(config) && mActDesc.loadString(action)) {
                ...
                Toast.makeText(this, R.string.trigger_invalid_settings, Toast.LENGTH_SHORT).show();
            }
        }

        // if there are any preselected actions specified when the activity is
        // first created
        // and there's currently nothing in the action description, load the
        // selected options
        // into the action description as if they were previously selected
        if (savedInstanceState == null && mActDesc.getCount() <= 0
                && getIntent().hasExtra(TriggerListActivity.KEY_PRESELECTED_ACTIONS)) {
            String[] preselectedActions = getIntent().getStringArrayExtra(
                    TriggerListActivity.KEY_PRESELECTED_ACTIONS);
    ...
    public void onResume() {
        ...
        TrigListPreference locPref = (TrigListPreference) getPreferenceScreen().findPreference(
                PREF_KEY_LOCATION);
        ...
        if (mCategories.length == 0) {
            ...
            if (locPref.getValue() == null
                    || !Arrays.asList(mCategories).contains(locPref.getValue())) {
        ...
        locPref.setOnCancelListener(new TrigListPreference.onCancelListener() {

            @Override
            public void onCancel() {
                new LocationTrigger().launchSettingsEditActivity(LocTrigEditActivity.this,
                        mAdminMode);
            }
        });
    }
    ...
    protected Dialog onCreateDialog(int id) {
        switch (id) {
            ...
            case DIALOG_ID_INVALID_TIME_ALERT:
                final String msgErr = "Make sure that the End Time is after the Start Time";

                return new AlertDialog.Builder(this).setTitle("Invalid time settings!")
                        .setNegativeButton("Cancel", null).setMessage(msgErr).create();

            case DIALOG_ID_NO_SURVEYS_SELECTED:
                final String msgErrNoSurveys = "Make sure that at least one survey is selected";

                return new AlertDialog.Builder(this).setTitle("No survey selected!")
                        .setNegativeButton("Cancel", null).setMessage(msgErrNoSurveys).create();

            case DIALOG_ID_DEFINE_LOC_PROMPT:
                final String msgPrompt = "'" + mTrigDesc.getLocation() + "' "
                        + "is not defined yet. Do you want to define it now?";

                return new AlertDialog.Builder(this).setTitle("Location not defined")
    ...
    private void initializeCategories() {
        ...
        if (c.moveToFirst()) {
            do {
                mCategories[i++] = c.getString(c.getColumnIndexOrThrow(LocTrigDB.KEY_NAME));
            } while (c.moveToNext());
        }
    ...
    private int getLocationCount(String categName) {
        ...
        if (cCateg.moveToFirst()) {
            categId = cCateg.getInt(cCateg.getColumnIndexOrThrow(LocTrigDB.KEY_ID));
        }
        cCateg.close();
    ...
    private void initializeGUI() {
        ListPreference locPref = (ListPreference) getPreferenceScreen().findPreference(
                PREF_KEY_LOCATION);
        CheckBoxPreference rangePref = (CheckBoxPreference) getPreferenceScreen().findPreference(
                PREF_KEY_ENABLE_RANGE);
        CheckBoxPreference alwaysPref = (CheckBoxPreference) getPreferenceScreen().findPreference(
                PREF_KEY_TRIGGER_ALWAYS);
        ...
        ((Button) findViewById(R.id.trig_edit_done)).setEnabled(mAdminMode
                || TrigUserConfig.editLocationTrigger);
    ...
    private void launchMapsActivity(String categName) {
        ...
        if (c.moveToFirst()) {
            int categId = c.getInt(c.getColumnIndexOrThrow(LocTrigDB.KEY_ID));
    ...
    private Dialog createEditActionDialog() {
        ...
        AlertDialog.Builder builder = new AlertDialog.Builder(this)
                .setTitle(R.string.trigger_select_actions).setNegativeButton(R.string.cancel, null)
                .setView(new ActionSelectorView(getBaseContext(), mActions, mActSelected));

        /*
         * AlertDialog.Builder builder = new AlertDialog.Builder(this)
         * .setTitle("Select surveys") .setNegativeButton("Cancel", null)
         * .setMultiChoiceItems(mActions, mActSelected, new
         * DialogInterface.OnMultiChoiceClickListener() {
         * @Override public void onClick(DialogInterface dialog, int which,
         * boolean isChecked) { mActSelected[which] = isChecked; } });
         */
